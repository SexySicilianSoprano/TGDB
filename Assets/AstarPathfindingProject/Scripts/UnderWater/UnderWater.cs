...
namespace Ceto
{
	...
	public class UnderWater : UnderWaterBase
	{
        ...
        /// </summary>
        public LayerMask oceanDepthsMask = 0;
        ...
        /// </summary>
		[HideInInspector]
        public Shader oceanBottomSdr, oceanMaskSdr, oceanDepthSdr, copyDepthSdr;
		...
		void Start () 
		{
			...
			try
			{
                ...
                willRender.AddAction(m_ocean.UpdateProjection);
		...
		void UpdateBottomBounds()
		{
			...
			{
				Mesh mesh = m_bottomMask.GetComponent<MeshFilter>().mesh;
				mesh.bounds = new Bounds(new Vector3(0.0f, level, 0.0f), new Vector3(bigNumber, OCEAN_BOTTOM_DEPTH, bigNumber));
			}
		...
		public override void RenderOceanMask(GameObject go)
        {
			...
			try
			{
                ...
                if (cam.name == "SceneCamera" || cam.GetComponent<UnderWaterPostEffect>() == null)
                {
		...
		public override void RenderOceanDepth(GameObject go)
        {
			...
			try
			{
                ...
                //zero the texture so if there is a issue the underwater effect will
                //not screw up the rest of the rendering.
                Shader.SetGlobalTexture(Ocean.REFRACTION_GRAB_TEXTURE_NAME, Texture2D.blackTexture);
                Shader.SetGlobalTexture(Ocean.DEPTH_GRAB_TEXTURE_NAME, Texture2D.whiteTexture);
                Shader.SetGlobalTexture("Ceto_OceanDepth", Texture2D.whiteTexture);
                ...
                else if (depthMode == DEPTH_MODE.USE_OCEAN_DEPTH_PASS)
                {
                    ...
                    data.depth.cam.cullingMask = oceanDepthsMask;
        ...
        void CreateMaskCameraFor(Camera cam, MaskData data)
        {
            ...
            if (data.cam == null)
            {
                ...
                GameObject go = new GameObject(Ocean.InstanceName + " Mask Camera: " + cam.name);
            ...
            if (tex == null || tex.width != cam.pixelWidth || tex.height != cam.pixelHeight)
            {
                ...
                data.cam.targetTexture.name = Ocean.InstanceName + " Mask Render Target: " + cam.name;
        ...
        void CreateDepthCameraFor(Camera cam, DepthData data)
        {
            ...
            if (data.cam == null)
            {
                ...
                GameObject go = new GameObject(Ocean.InstanceName + " Depth Camera: " + cam.name);
                go.hideFlags = HideFlags.HideAndDontSave;
				go.AddComponent<DisableFog>();
				go.AddComponent<DisableShadows>();
            ...
            if (tex == null || tex.width != cam.pixelWidth || tex.height != cam.pixelHeight)
            {
				...
				data.cam.targetTexture.hideFlags = HideFlags.DontSave;
                data.cam.targetTexture.name = Ocean.InstanceName + " Ocean Depths Render Target: " + cam.name;
        ...
        Mesh CreateBottomMesh(int segementsX, int segementsY)
        {
            ...
            mesh.name = Ocean.InstanceName + " Bottom Mesh";
