...
public class MinimapTrapezoid : MonoBehaviour {
	...
	public void Update() {
		...
		if (this.floorCollider.Raycast(topLeftCorner, out hits[0], 200f)) {
			this.topLeftPosition = hits[0].point;
		}
		if (this.floorCollider.Raycast(topRightCorner, out hits[1], 200f)) {
			this.topRightPosition = hits[1].point;
		}
		if (this.floorCollider.Raycast(bottomLeftCorner, out hits[2], 200f)) {
			this.bottomLeftPosition = hits[2].point;
		}
		if (this.floorCollider.Raycast(bottomRightCorner, out hits[3], 200f)) {
